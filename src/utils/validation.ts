...
export const blogPostSchema = z.object({
  ...
  tags: z.string().trim().max(200, "Tags string is too long").optional().or(z.literal("")),
  author: z.string().trim().min(1, "Author is required").max(100, "Author name is too long"),
  status: z.enum(["draft", "published"]),
  seoTitle: z.string().trim().max(200, "SEO title is too long").optional().or(z.literal("")),
  seoDescription: z.string().trim().max(500, "SEO description is too long").optional().or(z.literal("")),
  seoKeywords: z.string().trim().max(300, "SEO keywords string is too long").optional().or(z.literal("")),
});
